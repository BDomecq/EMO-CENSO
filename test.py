...
def test(data,
    ...
    # Half
    half = device.type != device and torch.cuda.device_count() == 1  # half precision only supported on single-GPU
    if half:
        model.half()  # to FP16
    ...
    if dataloader is None:  # not training
        ...
        _ = model(img.half() if half else img) if device.type != device else None  # run once
